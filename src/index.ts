let loadedAnswers : Array<number> = []

window.onload = function loadQuestions() {
    const link = <HTMLInputElement>document.getElementById("link")
    link.disabled = true
    ...
    for (let i = 0; i < toLoad.length; i++) {
        const element = <HTMLElement>toLoad[i];
        ...
        const questiondesc = <HTMLElement>element.children[0];
        ...
        (options[0] as HTMLElement).insertAdjacentText('beforeend', question.answer0);
        (options[2] as HTMLElement).insertAdjacentText('beforeend', question.answer1);
        (options[4] as HTMLElement).insertAdjacentText('beforeend', question.answer2);
    }
  };

function GetQuestionType (num: number): string{
    switch (num){
        case 0:
            return "crimson"
        case 1:
...
function submitAnswers(){
    ...
    if(button.textContent == "Potvrdit"){
        const submitedAnswers = document.querySelectorAll<HTMLInputElement>(".options input[type='radio']:checked")
